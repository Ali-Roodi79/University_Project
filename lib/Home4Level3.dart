...
import 'package:nullbox/main.dart';
import 'package:panorama_viewer/panorama_viewer.dart';
import 'dart:math' as math;
import 'package:subtitle/subtitle.dart';
import 'package:video_360/video_360.dart';
int scan=0;
final StreamController<String> streamController=new StreamController<String>.broadcast();
String subtitle="hi";
String Messages="";
String destination="";
AudioPlayer? _player;
bool sbool=false;
String res="";
class Home4Level3 extends StatefulWidget{
  @override
...
var icewidth=false;
bool Not=false;
class HomeState3 extends State<Home4Level3> with TickerProviderStateMixin{
  Video360Controller? controller22;
  ...
  void substate(int subb){
    setState(() {
      subtitle=subb.toString();
    });
  }
  double _lon = 0;
  double _lat = 0;
  ...
  List<Image> panoImages = [
    Image.asset('assets/vr/untitled.jpg'),
      ...
      body: Container(
        ...
        child: Stack(
          children: [
            ...
            //     url:
            //     'https://s3b-assets-bucket.s3.amazonaws.com/test-videos/360_VR+Master+Series+_+Free+Download+_+Crystal+Shower+Falls.mp4',
            ...
            PanoramaViewer(
              ...
              hotspots: [
                Hotspot(
                  ...
                  width: 90,
                  height: 75,
                  widget: hotspotButton(icon: Icons.wifi_calling_rounded, onPressed:(){
                    _scanfinalCharacter("");
                ...
                Hotspot(
                  ...
                  width: 60.0,
                  height: 60.0,
                  widget: hotspotButton(icon: Icons.back_hand, onPressed: () => setState(() => Navigator.pushNamed(context, "/quiz"))),
                ),
              ],
            ),
            Positioned(child:  Transform.rotate(angle: math.pi / 2,child: GestureDetector(
              child: SizedBox(
            ...
            ),),top: size.height*0.91,right: size.width*0.69,),
            Positioned(child:  Transform.rotate(angle: math.pi /2,child: GestureDetector(
              child: SizedBox(
                width: size.width*0.1,
                height: size.width*0.1,
            ...
            ),),top: size.height*0.91,right: size.width*0.57,),
            Positioned(child: Transform.rotate(angle:  math.pi /2,child: Center(
                      ...
                      child: Row(
                        ...
                        children: [
                          Visibility(child: SizedBox(
                            ...
                            child: Icon(Icons.mail,color: Colors.black,size: 18,),
                          ),visible: Not,),
                          SizedBox(
                            width: size.width*0.04,
                          ),
                          Text(Clock,style: TextStyle(color: Colors.black,fontSize: 18,decoration: TextDecoration.none),),
                          SizedBox(
                            width: size.width*0.04,
                          ),
                          Visibility(child: SizedBox(
            ...
            ),),left: size.width*0.79,top: size.height*0.13,),
            Positioned(child: Transform.rotate(angle: math.pi/2,
            ...
            ),top: size.height*0.9,right: size.width*0.81,),
            Positioned(child: StreamBuilder(
              initialData: " ",
              stream: streamController.stream,
              builder: (context, snapshot){
                return Transform.rotate(angle: math.pi/2,child: Text("${snapshot.data}",style: TextStyle(color: Colors.white,fontSize: 15, fontFamily: 'kalame'),maxLines:4,overflow: TextOverflow.ellipsis,),);
              }
            ...
            //     }
            //   },
            // ),top: size.height*0.48,right: size.width*0.12,),
            // Positioned(child: GestureDetector(
            //   child: Container(
            //     width: size.width*0.2,
            //     height: size.width*0.2,
  ...
  void MessageStructer(String cl){
    if(mesLevel3.containsKey(cl)){
      var data=mesLevel3[cl];
      var me;
      if(data![0]==day){
        me=mes2Level3[data![1]];
        ExportSnackBar(data?[1],me?[1]);
        mes2Level3.remove(data[1]);
        print(mes2Level3);
      }
...
void play(String path) {
  ...
  player.play(AssetSource("audio/$path.mp3"));
...
void sub(List sub){
  ...
  for ( int i=0 ; i<=lel;i++){
    if(i%3==0 || i==0){
      Timer(
          Duration(seconds: sub[i],milliseconds: sub[i+1]),(){
        String n=sub[i+2];
        print(n);
        streamController.sink.add(subtitle=n);
      }
      );
